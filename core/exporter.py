...
class Exporter:
    ...
    def generate_subscription(nodes: List[ProxyNode]) -> str:
        ...
        # Заголовки, которые поймут NekoBox / v2rayNG
        headers = [
            "#profile-title: SunnyAreral",
            "#profile-update-interval: 4",
            "#announce: Сервера обновляются каждые 4 часа. Поддержка: @SunnyAreral"
        ]
        
        # Соединяем заголовки и ссылки, затем кодируем всё это в Base64
        full_text = "\n".join(headers + links)
